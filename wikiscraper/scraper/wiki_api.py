...
BASE_REVISION_REQUEST = dict(BASE_REQUEST, **{"prop": "revisions", "rvslots": "main", "rvdir": "newer", })
...
def get_all_revisions(page_title: str, start_date: datetime) -> list[Revision]:
    params = dict(BASE_REVISION_REQUEST,
                  **{"titles": page_title,
                     "rvlimit": "500",
    ...
    for result in query(params):
        ...
        for page in pages:
            for rev in page["revisions"]:
                revisions.append(
                    Revision(
                        rev["revid"], rev["userid"], rev["timestamp"], rev["tags"]
                    )
                )
    return revisions
